...
class PeriodAggCsvWriter with CsvSchemaOld, CsvWriter<PeriodAgg> {
  final GroupFreq _f;
  ...
  String toRow(PeriodAgg rec) {
    return [Fmt.date(rec.dt, f: _f), ...rec.agg.map((d) => Fmt.durationHm(d))].join(sep);
  }
}

/// Writes CSV with two columns: time and total
class PeriodAggTotCsvWriter with CsvSchemaOld, CsvWriter<PeriodAgg> {
  final GroupFreq _f;
