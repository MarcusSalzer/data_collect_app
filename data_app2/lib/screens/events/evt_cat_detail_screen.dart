...
import 'package:data_app2/screens/events/cat_members_screen.dart';
...
class EvtCatDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => EvtCatDetailVm(stored, context.read<AppState>().db),
      child: Consumer<EvtCatDetailVm>(
        builder: (context, vm, _) {
          return EditScaffoldForVm<EvtCatRec>(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        if (vm.isDefault) Text("This is the default category"),
        if (storedCat != null)
          TextButton(
            onPressed: () {
              Navigator.of(context)
                  .push(
                    MaterialPageRoute(
                      builder: (context) => CatMembersScreen(storedCat),
                    ),
                  )
                  .then(
                    (value) {
